...
#include <aeroquad/nonlinearity.h> //TJARK
#include <aeroquad/gains.h> //TJARK
#include <cmath>
#include <iostream>
...
private:
  ...
  std::vector <double> errorsy_; // Previous errors in yst
...
public:
  Control()
  {
    ...
    velocity_publisher_ = node_handle_.advertise<geometry_msgs::TwistStamped>("cmd_vel_controller", 1);
  ...
  void gainCallback(const aeroquad::gains msg)
  {
  ...
  void nonlinCallback(const aeroquad::nonlinearity msg)
  {
      nonlinx = msg.x;
      nonliny = msg.y;
      nonlinz = msg.z;
  ...
  void publishControl()
  {
    ...
    */
    ...
    qx = wx;//wx*-cos(yaw_) + -wy*sin(yaw_);
    qy = wy;//wx*sin(yaw_) + wy*-cos(yaw_);
    ...
    velocity_.twist.linear.x = qx;
    velocity_.twist.linear.y = qy;
    velocity_.twist.linear.z = qz;
    velocity_.twist.angular.z = qyaw;
    ...
    if(velocity_.twist.linear.x > LIMIT) { velocity_.twist.linear.x = LIMIT;}
    if(velocity_.twist.linear.y > LIMIT) { velocity_.twist.linear.y = LIMIT;}
    if(velocity_.twist.linear.z > LIMIT) { velocity_.twist.linear.z = LIMIT;}
    if(velocity_.twist.linear.x < -LIMIT) { velocity_.twist.linear.x = -LIMIT;}
    if(velocity_.twist.linear.y < -LIMIT) { velocity_.twist.linear.y = -LIMIT;}
    if(velocity_.twist.linear.z < -LIMIT) { velocity_.twist.linear.z = -LIMIT;}

    if(velocity_.twist.angular.x > LIMIT) { velocity_.twist.angular.x = LIMIT;}
    if(velocity_.twist.angular.y > LIMIT) { velocity_.twist.angular.y = LIMIT;}
    if(velocity_.twist.angular.z > LIMIT) { velocity_.twist.angular.z = LIMIT;}
    if(velocity_.twist.angular.x < -LIMIT) { velocity_.twist.angular.x = -LIMIT;}
    if(velocity_.twist.angular.y < -LIMIT) { velocity_.twist.angular.y = -LIMIT;}
    if(velocity_.twist.angular.z < -LIMIT) { velocity_.twist.angular.z = -LIMIT;}
    ...
    velocity_.twist.linear.x = velocity_.twist.linear.x;
    velocity_.twist.linear.y = velocity_.twist.linear.y;
    velocity_.twist.linear.z = velocity_.twist.linear.z;



    //ROS_INFO("vx: %f, yaw: %f", velocity_.twist.angular.z);

    std_msgs::Float32 tot_x;
    tot_x.data = velocity_.twist.linear.x;
