...
private:
	...
	std::vector <geometry_msgs::PoseStamped> poses_; 	// Previous poses
...
public:
	StateEstimation()
	{
		...
		pose_publisher_ = node_handle_.advertise<geometry_msgs::PoseStamped>("filtered_pose",1);
	...
	void poseCallback(const geometry_msgs::PoseStamped pose)
	{
		...
		if(poses_.size() > 1)
		{
			...
			{
				windowSpeed_x = (poses_[n-1].pose.position.x - poses_[n-1-samplesBack].pose.position.x)/(times_[n-1] - times_[n-1-samplesBack]);
				windowSpeed_y = (poses_[n-1].pose.position.y - poses_[n-1-samplesBack].pose.position.y)/(times_[n-1] - times_[n-1-samplesBack]);
				windowSpeed_z = (poses_[n-1].pose.position.z - poses_[n-1-samplesBack].pose.position.z)/(times_[n-1] - times_[n-1-samplesBack]);
				...
				{
					if((std::abs(poses_[n-1-samplesBack].pose.position.x + windowSpeed_x*(times_[i]-times_[n-1-samplesBack])) > std::abs(poses_[i].pose.position.x+uncertainty_band_))
						&& (std::abs(poses_[n-1-samplesBack].pose.position.y + windowSpeed_y*(times_[i]-times_[n-1-samplesBack])) > std::abs(poses_[i].pose.position.y+uncertainty_band_))
						&& (std::abs(poses_[n-1-samplesBack].pose.position.z + windowSpeed_z*(times_[i]-times_[n-1-samplesBack])) > std::abs(poses_[i].pose.position.z+uncertainty_band_)))
					{
			...
			vel_.linear.x = (poses_[n-1].pose.position.x - poses_[n-1-samplesBack+1].pose.position.x)/(times_[n-1] - times_[n-1-samplesBack+1]);
			vel_.linear.y = (poses_[n-1].pose.position.y - poses_[n-1-samplesBack+1].pose.position.y)/(times_[n-1] - times_[n-1-samplesBack+1]);
			vel_.linear.z = (poses_[n-1].pose.position.z - poses_[n-1-samplesBack+1].pose.position.z)/(times_[n-1] - times_[n-1-samplesBack+1]);
